...
$show_my_posts_only = isset($_GET['show_my_posts_only']) ? $_GET['show_my_posts_only'] : false;
...
if ($show_my_posts_only) {
    $sql .= " AND b.username = :current_user";
...
if ($status == false) {
    ...
    while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        if ($result['username'] !== $_SESSION['username']) {
            ...
            $view .= '<button class="btn btn-helpful btn-block mb-2" disabled>';
  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item">
          <a class="nav-link text-white small-nav-text" href="index2.php"><i class="fas fa-database"></i> 解決本登録</a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-white small-nav-text" href="gpt.php"><i class="fas fa-search"></i> 悩み解決本検索</a>
        </li>
        <li class="nav-item">
...
<div class="container">
  <h2 class="text-center mt-4 mb-4 font-weight-bold"><i class="fas fa-book-open"></i>登録データ一覧</h2>
  ...
  <form action="" method="GET" class="mb-4" id="searchForm">
    ...
    <div class="sort-container mt-2">
        ...
        <button type="submit" name="show_my_posts_only" value="<?= $show_my_posts_only ? '0' : '1' ?>" class="btn btn-my-posts <?= $show_my_posts_only ? 'active' : '' ?>">
            <i class="fas fa-user"></i> 
            <?= $show_my_posts_only ? '全ての投稿を表示' : '自分の投稿のみ表示' ?>
        </button>
    </div>
  </form>
...
document.getElementById('resetSearch').addEventListener('click', function() {
    ...
    var myPostsButton = document.querySelector('button[name="show_my_posts_only"]');
    myPostsButton.value = '0';
    myPostsButton.innerHTML = '<i class="fas fa-user"></i> 自分の投稿のみ表示';
...
$(document).ready(function() {
    $('.helpful-button').on('click', function(e) {
        ...
        if (button.prop('disabled')) {
                    alert('自分の投稿には「助かりました」ボタンを押せません。');
                    return;
                }
...
$(document).on('click', '.send-message-btn', function() {
    ...
    if (receiverUsername === '<?php echo $_SESSION['username']; ?>') {
        alert('自分の投稿にはメッセージを送れません。');
        return;
    }
