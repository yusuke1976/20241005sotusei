    ...
    <body>
        ...
        <main>
            <section class="text-center container">
                ...
                allow="microphone">
                    ...
                    <div id="db-results" class="mt-4"></div>
        ...
        function handleGenreSelection(genre) {
            ...
            if (genre === 'ビジネス書') {
                ...
                searchTavily(concern + ' 自己啓発書 site:amazon.co.jp');
            } else if (genre === '小説') {
                searchTavily(concern + ' 小説 似た状況 site:ebookjapan.yahoo.co.jp');
            } else if (genre === '漫画') {
                searchTavily(concern + ' 漫画 site:cmoa.jp');
            } else {
                        // 他のジャンルの処理（必要に応じて実装）
            }
        }
        ...
        function searchTavily(query) {
            ...
            console.log('Searching Tavily with query:', query);  // デバッグログ

            fetch('tavily_search2.php', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                ...
                displayResults(data);
                displayDatabaseResults(data.db_results);
            })
            .catch(error => {
                console.error('Error:', error);
                errorDiv.textContent = `エラーが発生しました: ${error.message}`;
                tavilyResultsDiv.style.display = 'none';
        ...
        function displayDatabaseResults(results) {
            ...
            dbResultsDiv.innerHTML = '<h4>データベースから見つかった類似の本:</h4>';

            if (results && results.length > 0) {
                results.forEach(result => {
                    dbResultsDiv.innerHTML += `
                        <div class="result-item">
                            <h5><a href="${result.url}" target="_blank">${result.book}</a></h5>
                            <p>悩み: ${result.worry}</p>
                            <p>コメント: ${result.coment}</p>
                            <p>類似度: ${result.similarity.toFixed(2)}%</p>
                        </div>
                    `;
                });
            } else {
                dbResultsDiv.innerHTML += '<p>データベースに類似の本が見つかりませんでした。</p>';
            }
        ...
        function displayResults(data) {
            const resultsDiv = document.getElementById('tavily-results');
            resultsDiv.innerHTML = '';

            // Display OpenAI's answer (AI推薦)
            if (data.answer) {
                const answerElement = document.createElement('div');
                answerElement.innerHTML = `<h4>AI推薦結果①:</h4><p>${data.answer}</p>`;
            ...
            if (data.results && data.results.length > 0) {
                ...
                resultsElement.innerHTML = '<h4>AI推薦結果②:</h4>';
                ...
                resultsDiv.appendChild(resultsElement);
            } else {
